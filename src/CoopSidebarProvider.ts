...
export class CoopSidebarProvider implements vscode.WebviewViewProvider {
  ...
  public async resolveWebviewView(webviewView: vscode.WebviewView): Promise<void> {
    ...
    // Cold start stays blank — do not re-chip leftover editor tabs after attach.
    // File chips come from user focusing an editor or opening a past thread.

    webviewView.onDidChangeVisibility(() => {
      if (webviewView.visible) {
        this.session.touch();
        // Webview focus clears activeTextEditor; do not steal chips from visible tabs.
      }
    });
  }
