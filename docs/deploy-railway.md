...
| **Coop-AI** service | Dockerfile build → `node dist/webhookServer.js` |
...
3. Railway creates a service from the repo — it may default to **Coop-AI** (matches the repo name)
...
Set these on the **Coop-AI** service (**Variables** tab). Source for integration keys: your local **`.env.backend`** (gitignored — copy manually; do not commit).
...
1. **Coop-AI** → **Settings** → confirm **Root Directory** is repo root and **Builder** is Dockerfile (from `railway.toml`)
...
1. **Coop-AI** → **Settings** → **Networking** → **Generate Domain** (temporary `*.up.railway.app`)
2. **Terminal** (Mac):
...
1. **Coop-AI** → **Settings** → **Networking** → **Custom Domain**
...
5. On **Coop-AI**: `ZOEKT_URL=http://<zoekt-private-host>:6070`
...
| `DATABASE_URL is required` | Add Postgres reference on **Coop-AI** service |
