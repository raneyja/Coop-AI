...
| GitLab | `GITLAB_APP_ID`, `GITLAB_APP_SECRET` (+ optional `GITLAB_BASE_URL`) | [GitLab OAuth App](https://gitlab.com/-/user_settings/applications) — scopes `api` and `write_repository`; callback `/v1/gitlab/app/callback` |
| Bitbucket | `BITBUCKET_APP_ID`, `BITBUCKET_APP_SECRET` | [Bitbucket OAuth consumer](https://bitbucket.org/account/settings/api/) — Repositories **Write**, Pull requests **Write**; callback `/v1/bitbucket/app/callback` |
