    ...
    <footer className="mt-16 space-y-8 border-t border-coop-border pt-10">
      {links && links.length > 0 && (
        <div>
          ...
          <ul className="mt-4 space-y-2">
            {links.map((link) => (
              <li key={link.href}>
                <Link href={link.href} className="text-sm text-coop-index hover:text-white">
                  {link.label} →
                </Link>
              </li>
            ))}
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        {prev ? (
          <Link
            href={`/docs/${prev.slug}`}
            className="coop-panel block p-4 transition hover:border-coop-index/40"
          >
            <p className="text-xs text-coop-muted">Previous</p>
            <p className="mt-1 text-sm font-medium text-white">{prev.title}</p>
          </Link>
        ) : (
          <div />
        )}
        {next ? (
          <Link
            href={`/docs/${next.slug}`}
            className="coop-panel block p-4 text-right transition hover:border-coop-index/40 sm:col-start-2"
          >
