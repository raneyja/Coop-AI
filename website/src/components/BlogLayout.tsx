  ...
  return (
    <article className="mx-auto max-w-3xl px-6 py-12 md:py-20">
      ...
      >
        ...
        {heroImage && (
        <div className="relative mt-8 aspect-[16/10] w-full overflow-hidden border border-coop-border bg-coop-surface/40">
      ...
      <header className={`border-b border-coop-border pb-10 ${heroImage ? "mt-8" : "mt-8"}`}>
        <p className="text-sm text-coop-muted">
          ...
          <span className="text-coop-index">{formatCategoryLabel(category)}</span>
        </p>
      ...
      <div className="prose prose-lg prose-invert prose-headings:font-semibold prose-headings:tracking-tight prose-a:text-coop-index prose-code:text-coop-index prose-img:my-10 prose-img:border prose-img:border-white/10 max-w-none pt-10">
        {children}
      </div>
    </article>
  );
