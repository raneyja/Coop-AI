...
6. **Create a pull request** — After Apply on a Use-repo file, click **Create pull request** on the patch card, or type **Create a PR** in chat. Chat also works if you typed in the file yourself (no Apply). On a local file the button stays disabled. Confirm branch, title, and notes — nothing is created on GitHub until you submit. See [Create pull request](/docs/create-pull-request).
